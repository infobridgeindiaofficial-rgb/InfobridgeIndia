// Flipkart GSTR-1 report processing.
// Ported from the old InfoBridgeIndia website's js/flipkart.js (see
...
// the DOM/event-handling wrapper was removed in favour of plain exported
// functions. The Flipkart Sales Report already carries SALE and RETURN
// event rows in one file, so (matching the old tool) only one upload is
// ever required here - never a separate Flipkart return file.
...
async function readFlipkartReport(file) {
  ...
  if (!rows.length) throw new Error("The Sales Report sheet does not contain data rows.");
...
function validateReport(rows) {
  ...
  const missing = REQUIRED_COLUMNS.filter((column) => !availableColumns.has(column));
  ...
  const usableRows = rows.filter((row) => {
    ...
    return eventType === "SALE" || eventType === "RETURN" || toNumber(row.taxable_value_final_invoice_amount_taxes) !== 0;
  });
  if (!usableRows.length) throw new Error("No usable Sale or Return transactions were found.");
}

function calculateGST(rows, reportPeriod) {
  const transactions = rows.map((row, index) => prepareTransaction(row, index + 2)).filter(Boolean);
  if (!transactions.length) throw new Error("No usable Flipkart transactions were found.");

  const summary = transactions.reduce((total, row) => {
  ...
  return { period: reportPeriod, summary, b2c: groupB2C(transactions), hsn: groupHSN(transactions), eco: groupECO(transactions, summary) };
}
...
function prepareTransaction(row, sourceRow) {
  ...
  const taxable = Math.abs(toNumber(row.taxable_value_final_invoice_amount_taxes));
  ...
  return {
    sourceRow, eventType, sign,
    orderId: cleanText(row.order_id), orderItemId: cleanText(row.order_item_id),
    invoiceNumber: cleanText(row.buyer_invoice_id), orderDate: parseReportDate(row.order_date),
    hsnCode: cleanText(row.hsn_code), uqc: "NOS-NUMBERS", gstRate, state: state || "Unknown", stateCode, sellerGSTIN,
    signedQuantity: round2(sign * quantity), signedTaxable: round2(sign * taxable),
    signedIGST: round2(sign * igst), signedCGST: round2(sign * cgst), signedSGST: round2(sign * sgst), signedCess: round2(sign * cess),
  };
}
...
function groupB2C(transactions) {
  ...
  transactions.forEach((row) => {
    ...
    if (!groups.has(key)) groups.set(key, { stateCode: row.stateCode, state: row.state, gstRate: row.gstRate, supplyType, taxableValue: 0, igst: 0, cgst: 0, sgst: 0, cess: 0 });
    const group = groups.get(key);
    group.taxableValue += row.signedTaxable;
    group.igst += row.signedIGST; group.cgst += row.signedCGST; group.sgst += row.signedSGST; group.cess += row.signedCess;
  });
...
function groupHSN(transactions) {
  ...
  transactions.forEach((row) => {
    ...
    const group = groups.get(key);
    group.quantity += row.signedQuantity; group.taxableValue += row.signedTaxable;
    group.igst += row.signedIGST; group.cgst += row.signedCGST; group.sgst += row.signedSGST; group.cess += row.signedCess;
  });
  return Array.from(groups.values())
    .map((group) => ({
      hsnCode: group.hsnCode || "Missing", uqc: group.uqc, totalQuantity: round2(group.quantity), totalTaxableValue: round2(group.taxableValue), gstRate: group.gstRate,
      igst: round2(group.igst), cgst: round2(group.cgst), sgst: round2(group.sgst), status: "ENTER IN PORTAL", cess: round2(group.cess),
    }))
...
function groupECO(transactions, summary) {
  const sellerGSTIN = transactions.map((row) => row.sellerGSTIN).find(Boolean) || "";
  const sellerStateCode = /^\d{2}/.test(sellerGSTIN) ? sellerGSTIN.slice(0, 2) : "";
  ...
  return [{ ecoGSTIN: operator.gstin, tradeName: operator.tradeName, netTaxable: summary.netTaxable, igst: summary.netIGST, cgst: summary.netCGST, sgst: summary.netSGST, cess: summary.netCess }];
}
...
function buildWorkbook(result) {
  ...
  workbook.Props = { Title: "InfoBridgeIndia Flipkart GST Workbook", Subject: "Flipkart GST Portal Ready Working Sheets", Author: "InfoBridgeIndia", Company: "InfoBridgeIndia", Comments: "Generated from the Flipkart Sales Report." };
  return workbook;
}
...
function detectPeriod(rows) {
  const dates = rows.map((row) => parseReportDate(row.order_date)).filter(Boolean);
...
/**
 * End-to-end Flipkart processing from a single Sales Report - it already
 * contains both SALE and RETURN event rows, so no separate return file is
 * ever requested (matching the old tool exactly).
 */
export async function processFlipkart({ salesFile, gstin, fp }) {
  if (!salesFile) throw new Error("Please select the Flipkart Sales Report.");
  if (!isExcelFile(salesFile.name)) throw new Error("Only .xlsx files are supported for the Flipkart report.");

  const result0 = await readFlipkartReport(salesFile);
  const rows = normalizeRows(result0.rows);
  validateReport(rows);

  const reportPeriod = detectPeriod(rows);
  const result = calculateGST(rows, reportPeriod);
  const workbook = buildWorkbook(result);

  const safePeriod = (reportPeriod || "Report").replace(/\s+/g, "_").replace(/[^A-Za-z0-9_-]/g, "");
  const workbookFileName = `InfoBridgeIndia_Flipkart_GST_${safePeriod}.xlsx`;

  const jsonData = buildGstr1Json(result.b2c, result.hsn, result.eco, gstin, fp);
  const jsonFileName = `GSTR1_${gstin}_${fp}_Flipkart.json`;

  const counts = rows.reduce((c, row) => {
    const eventType = cleanText(row.event_type).toUpperCase();
    if (eventType === "SALE") c.sales += 1;
    if (eventType === "RETURN") c.returns += 1;
    return c;
  }, { sales: 0, returns: 0 });

  return {
    meta: { sheetName: result0.sheetName, salesRowCount: counts.sales, returnRowCount: counts.returns, period: reportPeriod },
