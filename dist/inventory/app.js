...
function openDb() {
  return createWorkspaceStore("inventory");
}

function all(collection) { return store.all(collection); }
function put(collection, value) { return store.put(collection, value); }
function remove(collection, id) { return store.remove(collection, id); }
function clear(collection) { return store.clear(collection); }
...
function setupScreen() {
  return `<main class="setup-shell"><section class="setup-card"><a href="/index.html"><img src="/infobridgeindia-logo.png" alt="InfoBridgeIndia" class="setup-logo"/></a><span class="eyebrow">Inventory & Warehouse setup</span><h1>Build inventory around your business</h1><p>Select a business type and InfoBridgeIndia will enable the fields that matter. You can adjust them later.</p><form id="setup-form"><label class="setup-select-label">Business Type<select class="input" name="businessType" required><option value="" selected disabled>Select your business type</option>${BUSINESS_TYPES.map((type) => `<option value="${type}">${type}</option>`).join("")}</select></label><div id="custom-fields-wrap" hidden><label>Custom fields <span class="hint">Comma separated</span></label><input class="input" name="customFields" placeholder="Grade, Pack Size, Material"/></div><div class="setup-actions"><button class="btn primary" type="submit">Create Inventory</button></div></form></section></main>`;
}
...
function movementTable(rows) {
  return `<div class="table-wrap"><table><thead><tr><th>Date / Transaction</th><th>Product</th><th>Type</th><th>Warehouse</th><th class="num">In</th><th class="num">Out</th><th class="num">Balance After</th><th>Reference</th><th>Action</th></tr></thead><tbody>${rows.map((m)=>`<tr class="movement-row" data-view-movement="${m.id}" tabindex="0" role="button"><td><strong>${dateTime(m.createdAt)}</strong><small class="mono">${escapeHtml(m.transactionId)}</small></td><td>${escapeHtml(product(m.productId)?.name||"Unknown product")}</td><td><span class="badge neutral">${escapeHtml(m.type.replaceAll("-"," "))}</span></td><td>${escapeHtml(warehouse(m.warehouseId)?.name||"Unknown")}</td><td class="num in">${m.quantityIn||"—"}</td><td class="num out">${m.quantityOut||"—"}</td><td class="num"><strong>${m.balanceAfter}</strong></td><td>${escapeHtml(m.reference||m.reason||"—")}</td><td><button class="text-button" data-view-movement-button="${m.id}">View</button></td></tr>`).join("")}</tbody></table></div>`;
}
function openDetailsModal(body){modalRoot.innerHTML=`<div class="modal-backdrop"><section class="modal" role="dialog" aria-modal="true" aria-labelledby="movement-details-title"><header><h2 id="movement-details-title">Movement Details</h2><button class="icon-button" data-close-modal aria-label="Close">${icon("close")}</button></header><div class="modal-body">${body}</div><footer><button class="btn primary" type="button" data-close-modal>Close</button></footer></section></div>`;modalRoot.querySelectorAll("[data-close-modal]").forEach(button=>button.addEventListener("click",()=>modalRoot.innerHTML=""))}
function movementDetails(id){const m=state.movements.find(item=>item.id===id);if(!m)return;const p=product(m.productId),pair=state.movements.find(item=>item.transactionId===m.transactionId&&item.id!==m.id),out=m.type==="transfer-out"?m:pair?.type==="transfer-out"?pair:null,inside=m.type==="transfer-in"?m:pair?.type==="transfer-in"?pair:null,transfer=m.type.startsWith("transfer-");openDetailsModal(`<div class="product-view"><dl><div><dt>Transaction ID</dt><dd class="mono">${escapeHtml(m.transactionId||m.id)}</dd></div><div><dt>Date and time</dt><dd>${escapeHtml(dateTime(m.createdAt))}</dd></div><div><dt>Product and SKU</dt><dd>${escapeHtml(p?.name||"Unknown product")} · ${escapeHtml(p?.sku||"—")}</dd></div><div><dt>Movement type</dt><dd>${escapeHtml(m.type.replaceAll("-"," "))}</dd></div><div><dt>Warehouse</dt><dd>${escapeHtml(warehouse(m.warehouseId)?.name||"Unknown")}</dd></div>${transfer?`<div><dt>From Warehouse</dt><dd>${escapeHtml(warehouse(out?.warehouseId)?.name||"Unknown")}</dd></div><div><dt>To Warehouse</dt><dd>${escapeHtml(warehouse(inside?.warehouseId)?.name||"Unknown")}</dd></div>`:""}<div><dt>Quantity in</dt><dd>${m.quantityIn||"—"}</dd></div><div><dt>Quantity out</dt><dd>${m.quantityOut||"—"}</dd></div><div><dt>Balance after</dt><dd>${m.balanceAfter}</dd></div><div><dt>Cost</dt><dd>${money(m.unitCost||0)}</dd></div><div><dt>Reference</dt><dd>${escapeHtml(m.reference||"—")}</dd></div><div><dt>Reason / Destination</dt><dd>${escapeHtml(m.reason||(transfer?warehouse(inside?.warehouseId)?.name||"—":"—"))}</dd></div><div><dt>Notes</dt><dd>${escapeHtml(m.notes||"No notes added.")}</dd></div></dl></div>`)}
...
function productForm(existing={}) {
  ...
  const openingFields=!existing.id?`${field("openingStock","Opening Stock","number",0,false,'step="0.001" min="0"')}<label>Opening Stock Warehouse<select class="input" name="warehouseId" required>${selectWarehouses()}</select></label>`:"";
...
function validateBackup(data){
  if(!data||typeof data!=="object"||data.version!==1||!STORES.every(store=>Array.isArray(data[store])))throw new Error("This is not a valid or compatible InfoBridgeIndia Inventory backup.");
  for(const store of STORES){const ids=new Set();for(const item of data[store]){if(!item||typeof item!=="object"||typeof item.id!=="string"||!item.id)throw new Error(`Backup contains an invalid ${store} record.`);if(ids.has(item.id))throw new Error(`Backup contains duplicate ${store} record IDs.`);ids.add(item.id)}}
  const setup=data.settings.find(item=>item.id==="app");if(!setup||typeof setup.businessType!=="string"||!setup.businessType)throw new Error("Backup does not contain a valid inventory business setup.");
  const productIds=new Set(data.products.map(item=>item.id));const warehouseIds=new Set(data.warehouses.map(item=>item.id));
  for(const movement of data.movements){if(!productIds.has(movement.productId)||!warehouseIds.has(movement.warehouseId)||!movement.type||!Number.isFinite(Number(movement.quantity)))throw new Error("Backup contains an invalid stock movement or missing product/warehouse reference.")}
  for(const count of data.counts){if(!productIds.has(count.productId)||!warehouseIds.has(count.warehouseId))throw new Error("Backup contains an invalid physical stock count reference.")}
  return data;
}
async function replaceAllInventoryData(data){for(const storeName of STORES)await store.replace(storeName,data[storeName])}
async function restore(file){let data;try{data=validateBackup(JSON.parse(await file.text()))}catch(error){if(error instanceof SyntaxError)throw new Error("The selected file is not valid JSON and no inventory data was changed.");throw error}if(!confirm("Restore this full backup? This will replace all current local Inventory data. This action does not merge datasets."))return;await replaceAllInventoryData(data);await reload();render();toast("Full backup restored. You can continue from the saved inventory state.")}
...
function bind(){
  document.querySelectorAll(".movement-row").forEach(row=>{const open=()=>movementDetails(row.dataset.viewMovement);row.addEventListener("click",event=>{if(!event.target.closest("[data-view-movement-button]"))open()});row.addEventListener("keydown",event=>{if(event.key==="Enter"||event.key===" "){event.preventDefault();open()}})});
  document.querySelectorAll("[data-view-movement-button]").forEach(button=>button.addEventListener("click",event=>{event.stopPropagation();movementDetails(button.dataset.viewMovementButton)}));
  document.querySelectorAll("[data-view]").forEach(b=>b.addEventListener("click",()=>{state.view=b.dataset.view;history.replaceState(null,"",`#${state.view}`);document.querySelector("#sidebar")?.classList.remove("open");render()}));
  ...
  document.querySelectorAll("[data-restore-file]").forEach(input=>input.addEventListener("change",e=>e.target.files[0]&&restore(e.target.files[0]).catch(err=>toast(err.message,"error"))));
...
try {
  ...
  if(nav.some(([view])=>view===requestedView)) state.view=requestedView;
  addEventListener("hashchange",()=>{const view=location.hash.slice(1);if(nav.some(([name])=>name===view)&&state.view!==view){state.view=view;render()}});
  render();
} catch(error) { app.innerHTML=`<main class="fatal"><h1>Inventory could not open</h1><p>${escapeHtml(error.message)}</p><p>Check your connection and sign-in, then try again.</p></main>`; }
