...
function calculateLateFee() {
  ...
  const delayDays = Math.round(
    (filingDate.getTime() - dueDate.getTime()) / millisecondsPerDay
  );
  ...
  const maximumFee = getMaximumLateFee(
    returnStatus,
    turnoverSlab
  );
  ...
  resultReturnType.textContent =
    returnType === "gstr1" ? "GSTR-1" : "GSTR-3B";

  delayDaysElement.textContent = `${delayDays} ${
...
returnStatusSelect.addEventListener(
  "change",
  updateTurnoverVisibility
);
